...
2. **자동 빌드 스크립트**: 동봉된 파워쉘 스크립트를 실행하면 vswhere를 통한 경로 탐색과 CMake 프리셋 빌드가 자동으로 진행됩니다.
   ...
   .\build_local.ps1
   ```
3. **프로파일링 모드 활성화 (선택)**: CMake 컴파일 시 `-DENABLE_PROFILING=ON`(기본값) 토글로 Tracy 프로파일러 컴파일을 켜고 끌 수 있습니다. 상세 연동 및 런북은 [04_profiling_and_optimization.md](https://github.com/jin20203458/Obsidian.Agent/blob/main/MundusVivens/docs/04_profiling_and_optimization.md)를 참조하십시오.
